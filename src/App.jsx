...
const Portfolio = () => {
  ...
  const [isResumeOpen, setIsResumeOpen] = useState(false); // State to control the resume modal
    ...
    <div className="min-h-screen bg-[#FDFDFD] font-sans text-slate-900 selection:bg-orange-100 selection:text-orange-900">
      ...
      <nav className="fixed top-0 w-full z-100 bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex gap-8 items-center">
          ...
          <a href="https://github.com/bolujolayemi-a11y" target="_blank" className="p-2 bg-slate-50 rounded-xl hover:bg-orange-500 hover:text-white transition-all">
            <User size={18} />
          </a>
        </div>
      </nav>
      ...
      {/* --- HERO SECTION --- */}
      <header className="px-6 md:px-20 pt-44 pb-20 max-w-7xl mx-auto">
        <motion.div 
        ...
        <div className="flex flex-wrap gap-4">
          <a href="#work" className="px-8 py-5 bg-slate-900 text-white rounded-3xl font-black uppercase tracking-widest text-[10px] hover:bg-orange-500 transition-all flex items-center gap-3 group shadow-xl shadow-slate-200">
            View My Projects
            <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </a>
          {/* Button correctly triggers the Resume Modal */}
          <button 
      ...
      <section className="bg-slate-50 py-24 px-6 md:px-20 border-y border-slate-100">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-500 mb-4">Flagship Project</h2>
            <h3 className="text-4xl md:text-6xl font-black tracking-tight mb-6">Pantry Guide<span className="text-slate-300">.</span></h3>
            ...
            <a href="https://my-pantry-guide.vercel.app/" target="_blank" className="inline-flex items-center gap-3 px-6 py-3 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:border-orange-500 transition-all">
              Live Demo <ExternalLink size={14} />
            </a>
          </div>
          
          <div className="relative">
             <div className="aspect-4/3 bg-orange-50 rounded-[40px] md:rounded-[60px] overflow-hidden border-8 border-white shadow-2xl relative z-10 p-6 flex items-center justify-center">
                <img 
                  src={demoMockup} 
      ...
      {/* --- DUAL WORK GRID --- */}
      <section id="work" className="px-6 md:px-20 py-32 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <div>
            <h2 className="text-5xl font-black tracking-tight mb-4">Selected Portfolio</h2>
            <p className="text-slate-400 font-medium italic">Toggle between Front-end Development and Data Analytics</p>
          </div>
          <div className="flex bg-slate-100 p-1.5 rounded-[20px] self-start">
            ...
            {activeTab === 'development' ? (
              ...
              <>
                <ProjectCard 
                  ...
                  desc="Mapping 10k+ transactions to identify supply chain gaps and seasonal trends."
      ...
      {/* --- FOOTER --- */}
      <footer className="bg-slate-900 text-white py-32 px-6 md:px-20 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-10">
             <a href="mailto:boluwatifejolayemi@gmail.com" className="text-xs font-black uppercase tracking-[0.4em] flex items-center gap-3 hover:text-orange-500 transition-colors">
               <Globe size={16} /> Lagos, Nigeria
             </a>
             <a href="https://linkedin.com/in/boluwatife-jolayemi-195593218" className="text-xs font-black uppercase tracking-[0.4em] flex items-center gap-3 hover:text-orange-500 transition-colors">
               <User size={16} /> Contact Me
             </a>
          </div>
          <div className="mt-24 pt-12 border-t border-white/5 text-[9px] font-black uppercase tracking-[0.5em] text-white/30">
...
const TabButton = ({ active, onClick, label, icon }) => (
  <button 
    onClick={onClick}
    className={`flex items-center gap-2 px-8 py-3 rounded-[14px] text-[10px] font-black uppercase tracking-widest transition-all ${active ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
  >
    {icon} {label}
  </button>
);
...
const ProjectCard = ({ title, desc, tags, icon, link }) => (
  <div className="group p-10 bg-white border border-slate-100 rounded-[50px] hover:border-orange-200 transition-all hover:shadow-2xl hover:shadow-orange-500/5 relative overflow-hidden flex flex-col justify-between h-full">
    <div>
      <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 mb-8 group-hover:bg-orange-500 group-hover:text-white transition-all">
        {icon}
      </div>
      <div className="flex gap-2 mb-4">
        {tags.map(tag => (
          <span key={tag} className="text-[9px] font-black uppercase tracking-widest text-slate-300 px-3 py-1 border border-slate-100 rounded-full">{tag}</span>
        ))}
      </div>
      <h4 className="text-2xl font-black tracking-tight mb-3 text-slate-900">{title}</h4>
      <p className="text-slate-500 font-medium leading-relaxed mb-6">{desc}</p>
    </div>
    {link && (
      <a href={link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-orange-600 hover:text-orange-700 transition-colors">
