...
export const WorkView = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8 text-left selection:bg-pink-500/20 selection:text-pink-400">
      ...
      {/* FILTER BUTTON TABS */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-[#1b0b30]/10 pb-6 mb-12">
        <div>
          <h2 className="text-3xl font-black text-[#1b0b30] tracking-tight">Selected Work</h2>
          <p className="text-[#1b0b30]/50 font-mono text-[10px] uppercase tracking-wider mt-1">Isolate system directories to view active operations</p>
        </div>
        
        <div className="flex bg-[#1b0b30]/5 border border-[#1b0b30]/10 p-1 rounded-xl w-max backdrop-blur-sm">
          <button 
            onClick={() => setActiveTab('development')} 
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-sans font-black uppercase tracking-wider transition-all duration-200 cursor-pointer ${
              activeTab === 'development' 
                ? 'bg-[#1b0b30] text-white shadow-md' 
                : 'text-[#1b0b30]/60 hover:text-[#1b0b30]'
            }`}
          >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeProjects.map((project, idx) => (
              ...
              >
                {project.isAI && (
                  <div className="absolute top-0 left-0 px-2.5 py-1 bg-pink-500/10 border-b border-r border-[#1b0b30]/10 rounded-tl-xl rounded-br-md z-20">
                    <span className="text-[8px] font-mono font-bold tracking-widest text-pink-600 dark:text-pink-300 uppercase">AI Integrated</span>
                  </div>
                )}
                ...
                <div className="lg:col-span-5 pt-12 p-6 sm:p-8 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-[#1b0b30]/10 relative">
                  ...
                  {/* RESTORED: Hardcoded dark purple fallback color alignment */}
                  ...
                  <div className="flex w-full mt-auto">
                    {project.liveLink ? (
                      ...
                      <div className="text-center w-full py-3 bg-[#1b0b30]/5 text-[#1b0b30]/40 rounded-xl font-mono text-[9px] uppercase tracking-wider border border-[#1b0b30]/5">
                        Private Study Node
                      </div>
                    )}
                  </div>
                ...
                <div className="lg:col-span-7 bg-[#1b0b30]/5 min-h-65 md:min-h-85 flex items-center justify-center relative overflow-hidden group">
                  {project.image ? (
                    <img 
                      src={project.image} 
                    ...
                    <div className="w-full h-full min-h-65 md:min-h-85 flex flex-col items-center justify-center text-[#1b0b30]/30 font-mono text-[9px] uppercase tracking-widest gap-2">
                      <div className="w-5 h-5 rounded-full border border-[#1b0b30]/10 flex items-center justify-center text-[8px] text-[#1b0b30]/40">
                        {idx + 1}
                      </div>
