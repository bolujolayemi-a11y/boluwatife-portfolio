...
export const WorkView = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8 text-left selection:bg-pink-500/20 selection:text-pink-400">
      ...
      {/* FILTER BUTTON TABS */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-white/10 pb-6 mb-12">
        <div>
          <h2 className="text-3xl font-black text-white tracking-tight">Selected Work</h2>
          <p className="text-pink-200/50 font-mono text-[10px] uppercase tracking-wider mt-1">Isolate system directories to view active operations</p>
        </div>
        
        <div className="flex bg-white/5 border border-white/10 p-1 rounded-xl w-max backdrop-blur-sm">
          <button 
            onClick={() => setActiveTab('development')} 
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-sans font-black uppercase tracking-wider transition-all duration-200 ${activeTab === 'development' ? 'bg-white text-[#1b0b30] shadow-md' : 'text-pink-200/60 hover:text-white'}`}
          >
            <Code size={12} /> Development ({developmentProjects.length})
          </button>
          <button 
            onClick={() => setActiveTab('analysis')} 
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-sans font-black uppercase tracking-wider transition-all duration-200 ${activeTab === 'analysis' ? 'bg-white text-[#1b0b30] shadow-md' : 'text-pink-200/60 hover:text-white'}`}
          >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeProjects.map((project, idx) => (
              <div 
                key={idx}
                className="grid grid-cols-1 lg:grid-cols-12 rounded-2xl overflow-hidden border border-white/10 hover:border-white/20 transition-all duration-200 shadow-xl relative bg-white/5 backdrop-blur-sm"
              >
                {project.isAI && (
                  <div className="absolute top-0 left-0 px-2.5 py-1 bg-pink-500/10 border-b border-r border-white/10 rounded-tl-xl rounded-br-md z-20">
                    <span className="text-[8px] font-mono font-bold tracking-widest text-pink-300 uppercase">AI Node</span>
                  </div>
                )}

                {/* LEFT CONTENT PANEL */}
                <div className="lg:col-span-5 p-6 sm:p-8 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-white/10 relative">
                  <div className="mb-4">
                    <h3 className="text-xl md:text-2xl font-black text-white tracking-tight mb-0.5">{project.title}</h3>
                    <span className="text-[11px] font-mono font-bold text-pink-300 uppercase tracking-wider">{project.category}</span>
                  </div>

                  <p className="text-white/70 text-xs md:text-sm leading-relaxed mb-6 font-normal">
                    {project.desc}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-8">
                    {project.tags.map((tag, tIdx) => (
                      <span 
                        key={tIdx} 
                        className="text-[9px] font-mono font-bold px-2 py-1 bg-white/5 text-pink-200/70 border border-white/5 rounded"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="flex w-full mt-auto">
                    {project.liveLink ? (
                      <a 
                        ...
                        target="_blank" 
                        rel="noreferrer" 
                        className="flex items-center justify-center gap-2 w-full py-3 bg-white text-[#1b0b30] rounded-xl font-sans font-black text-[10px] uppercase tracking-widest hover:bg-pink-100 transition-colors shadow-md shadow-black/5 text-center"
                      >
                        Live Demo <ExternalLink size={11} />
                      </a>
                    ) : (
                      <div className="text-center w-full py-3 bg-white/5 text-white/40 rounded-xl font-mono text-[9px] uppercase tracking-wider border border-white/5">
                        Private Study Node
                      </div>
                    )}
                  </div>
                ...
                <div className="lg:col-span-7 bg-black/10 min-h-65 md:min-h-85 flex items-center justify-center relative overflow-hidden group">
                  {project.image ? (
                    <img 
                      src={project.image} 
                    ...
                    <div className="w-full h-full min-h-65 md:min-h-85 bg-black/10 flex flex-col items-center justify-center text-white/30 font-mono text-[9px] uppercase tracking-widest gap-2 shadow-inner group-hover:border-white/5 transition-colors">
                      <div className="w-5 h-5 rounded-full border border-white/10 flex items-center justify-center text-[8px] text-white/40">
                        {idx + 1}
                      </div>
